...
/** Ofuz Open Source version is released under the GNU Affero General Public License, please read the full license at: http://www.gnu.org/licenses/agpl-3.0.html **/
    ...
    $uploaded_thumb_file = $this->getParam("uploaded_thumb_files");
    $thumb_image_width = $this->getParam("thumb_maxwidth");
    $thumb_image_height = $this->getParam("thumb_maxheight");



//    print_r($uploaded_file);
//    print_r($uploaded_thumb_file);exit;
    $this->setLog("\n Got Param:".count($uploaded_file));
    $this->setLog("\n Got thumb Param:".count($uploaded_thumb_file));
    ...
    //crop tumbfiles
    foreach($uploaded_thumb_file as $field_name => $imagefilename) {
        if (function_exists("imagecreatetruecolor")) {
            $this->setLog("\n processing field:".$field_name." image:".$imagefilename);
            if ((!empty($thumb_image_width[$field_name]) || !empty($thumb_image_height[$field_name])) && !empty($imagefilename)) {
                $max_width = $thumb_image_width[$field_name];
                $max_height = $thumb_image_height[$field_name];
                list($width, $height) = getimagesize($imagefilename);
                $height_coef = $height / $width;
                $width_coef = $width / $height;
                if ($width > $max_width && $new_width > 0) { 
