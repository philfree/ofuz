...
class OfuzExportAccount extends DataObject {
  private $file_handle;
  
  function eventExportUserContacts(EventControler $evtcl) {

    $xml_file = "xml_export/".$_SESSION['do_User']->iduser."_contacts.xml";
    $this->setFileHandle(fopen($xml_file, "w+"));
    if ($this->XMLExport($_SESSION['do_User']->iduser)) {
  ...
  private function setFileHandle($file_handle) {
...
/**
  ...
  private function XMLExport($iduser) {
    ...
    if (!$this->appendToFile($xml)) { return false; };
    $xml = ' ';
